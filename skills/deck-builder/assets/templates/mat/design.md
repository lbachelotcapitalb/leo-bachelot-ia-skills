...
|---|---|---|---|---|
| `{typography.display}` | 230.4px | Bricolage Grotesque | 800 | Cover or opening hero display — massive, line-broken into 1–2 words per line |
| `{typography.h1}` | 134.4px | Bricolage Grotesque | 800 | Chapter-opening or closing headline |
| `{typography.h2}` | 76.8px | Bricolage Grotesque | 700 | Primary content-slide headline |
| `{typography.stat-value}` | 105.6px | Bricolage Grotesque | 800 | Large numerical figure inside a stat cell |
| `{typography.quote-mark}` | 153.6px | Bricolage Grotesque | 800 | Decorative opening quotation glyph in warm orange |
| `{typography.quote-text}` | 65.3px | Bricolage Grotesque | 600 | Pull-quote body text |
| `{typography.h3}` | 46.1px | Bricolage Grotesque | 600 | Sub-headline or region heading |
| `{typography.lead}` | 28.8px | DM Sans | 400 | Lead paragraph or large bullet item |
| `{typography.body}` | 20.2px | DM Sans | 400 | Body paragraph and info-card body |
| `{typography.caption}` | 15.7px | DM Sans | 400 | Image caption, tagline, fine print |
| `{typography.label}` | 13.4px | DM Mono | 400 | Kicker, chrome tag, footer label, metadata, chart label |

### Defaults

- **Default primary section headline**: `{typography.h2}` (76.8px).
- **Default opening or cover display**: `{typography.display}` (230.4px) on a cover; `{typography.h1}` (134.4px) on a chapter or closing moment.
- **Default body paragraph size**: `{typography.body}` (20.2px). Reach for `{typography.lead}` (28.8px) when the paragraph is the single supporting block under a large headline.
- **Default label / kicker size**: `{typography.label}` (13.4px).
...
Mat is **viewport-fluid by design**. All sizes use `vw` and `vh` units so the layout scales linearly with the viewport. The slide container is `1920px × 1080px` and the deck is a horizontal flex strip that translates left/right by full viewport widths. There is no fixed canvas dimension; the same composition renders correctly on any 16:9-ish viewport.

### Padding and Gap Scale

| Token | Value | Use |
|---|---|---|
| `{spacing.pad-x}` | 105.6px | Slide horizontal padding; the breathing room on left and right edges |
| `{spacing.pad-y}` | 59.4px | Slide vertical padding |
| `{spacing.gap-lg}` | 48.6px | Between major content sections |
| `{spacing.gap-md}` | 30.2px | Between related elements (headline → lead, stat → label) |
| `{spacing.gap-sm}` | 15.1px | Between tightly related elements (label pair in chrome band) |
...
**Kicker** — A small DM Mono uppercase tag in warm orange with 0.12em positive tracking, placed above a headline as an eyebrow label. The most consistent appearance of orange in the system. Typically 13.4px size.
...
Mat targets a fluid `1920px × 1080px` viewport and uses `vw`/`vh` units for every typographic size, padding, and gap. The same composition renders correctly across 1280×720, 1920×1080, and 2560×1440 displays without breakpoints. Because all sizes are viewport-relative, content scales linearly — at wider viewports, both text and padding grow proportionally, so the visual density stays constant.
...
|---|---|---|---|
| Display / h1 / h2 / h3 / stat-value (Bricolage roles, 2.4–230.4px) | 思源宋体 Noto Serif SC | 700 | Mincho heavy weight provides the structural mass that Bricolage 700–800 provides in Latin, while preserving the warm material register |
| Quote text / quote mark (65.3px / 153.6px) | 思源宋体 Noto Serif SC | 600–700 | Same Mincho voice for editorial moments |
| Lead / body / caption (DM Sans roles, 0.82–28.8px) | 思源宋体 Noto Serif SC | 400 | Mincho body voice — calm and material; pairs with the wood-glow warmth |
| Info-card body / heading | 思源宋体 Noto Serif SC | 400 / 600 | Maintains the warm-paper-on-green contrast in Chinese |
| Label / kicker / chrome / footer (DM Mono roles, 13.4px) | 思源宋体 Noto Serif SC | 500 with 0.05em letter-spacing | Replaces DM Mono — Chinese has no monospace tradition that reads as editorial chrome |
...
- The `.split-center` image region is fixed at 561.6px height. Tall portrait imagery may need a layout-level override.
