...
|---|---|---|
| `display` | 182.4 px | **271 px** |
| `quote-mark` | 153.6 px | **228 px** |
| `stat-value` | 105.6 px | **157 px** |
| `h1` | 99.8 px | **148 px** |
| `quote-text` | 69.1 px | **103 px** |
| `h2` | 57.6 px | **86 px** |
| `editorial-headline` | 52.8 px | **78 px** |
| `dense-headline` | 46.1 px | **68 px** |
| `h3` | 36.5 px | **54 px** |
| `lead` | 26.9 px | **40 px** |
| `body` | 20.2 px | **30 px** |
...
|---|---|---|---|---|
| `{typography.display}` | 182.4px | Source Serif 4 | 700 | Hero cover headline at maximum scale |
| `{typography.h1}` | 99.8px | Source Serif 4 | 600 | Chapter title or single-statement headline |
| `{typography.stat-value}` | 105.6px | Source Serif 4 | 600 | Large statistical numeral in gold |
| `{typography.quote-mark}` | 153.6px | Source Serif 4 | 300 | Decorative opening quotation glyph in gold |
| `{typography.quote-text}` | 69.1px | Source Serif 4 | 400 | Pull-quote body |
| `{typography.h2}` | 57.6px | Source Serif 4 | 600 | Primary slide headline |
| `{typography.editorial-headline}` | 52.8px | Source Serif 4 | 600 | Headline for editorial / newsletter layouts |
| `{typography.dense-headline}` | 46.1px | Source Serif 4 | 600 | Headline for two-column dense text layouts |
| `{typography.h3}` | 36.5px | Source Serif 4 | 500 | Sub-headline, panel title, intra-region heading |
| `{typography.lead}` | 26.9px | DM Sans | 400 | Lead paragraph, intro sentence, list item body |
| `{typography.body}` | 20.2px | DM Sans | 400 | Paragraph body, bullet body |
| `{typography.caption}` | 15.7px | DM Sans | 400 | Caption, footnote, source attribution |
| `{typography.label}` | 13.4px | IBM Plex Mono | 500 | Mono uppercase chrome label, kicker, tag, mono metadata |

### Defaults

- **Default size for a primary section headline**: `{typography.h2}` (57.6px).
- **Default size for a chapter opener**: `{typography.h1}` (99.8px).
- **Default size for a cover hero**: `{typography.display}` (182.4px).
- **Default size for a paragraph**: `{typography.body}` (20.2px).
- **Default size for a lead sentence beneath a headline**: `{typography.lead}` (26.9px).
- **Default size for any chrome, kicker, or metadata label**: `{typography.label}` (13.4px).
- **Default size for a statistical numeral**: `{typography.stat-value}` (105.6px), in `{colors.gold}`.
...
The system targets `1920px × 1080px` — full viewport. Each `.slide` flexes to fill the viewport exactly, and slides sit side-by-side in a horizontal strip that translates left/right on navigation. All sizes use viewport-relative units (`vw`, `vh`) so the layout scales with window size — there are no fixed pixel measurements except chrome dot/counter sizes.

### Padding and Gap Scale

| Token | Value | Use |
|---|---|---|
| `{spacing.pad-x}` | 144px | Horizontal slide padding |
| `{spacing.pad-y}` | 59.4px | Vertical slide padding |
| `{spacing.gap-lg}` | 43.2px | Between major content sections |
| `{spacing.gap-md}` | 27px | Between related elements |
| `{spacing.gap-sm}` | 13px | Between tightly coupled elements |
...
- Pad with `{spacing.pad-x}` 144px / `{spacing.pad-y}` 59.4px — Signal needs the breathing room; tight padding breaks the editorial restraint.
...
- Display headline scales with viewport: at 1920px, 182.4px renders as ~182px. At 1280px, ~122px.
- Body text scales: 20.2px → ~20px at 1920px, ~13px at 1280px.
- Padding scales: pad-x 144px → ~144px at 1920px, ~96px at 1280px.
...
- **Line-height on CJK body should open from 1.58–1.72 to 1.75–1.85** — Chinese characters fill their em-box and need additional vertical breathing for readability at the system's body sizes (20.2px, 26.9px).
- **Quote-mark glyph (the 153.6px decorative opening quotation)** should switch to a Chinese opening guillemet (「) or curly quote (") — Latin opening-quote glyphs read as alien on Chinese pull-quotes.
