...
This model was authored fluid for a ~1024 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.875** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.
...
|---|---|---|
| `stat-big` | 320 px | **320 px** — unchanged |
| `hero-title-red` | 260 px | **260 px** — unchanged |
| `close-big` | 260 px | **260 px** — unchanged |
| `hero-title` | 220 px | **412 px** |
| `hero-title-bottom` | 200 px | **375 px** |
| `red-quote` | 90 px | **169 px** |
| `section-header-lg` | 72 px | **135 px** |
| `section-header` | 64 px | **120 px** |
| `pillar-num` | 64 px | **120 px** |
| `stat-item-num` | 56 px | **105 px** |
| `cell-number-lg` | 52 px | **98 px** |
| `cell-number-md` | 48 px | **90 px** |
| `card-title` | 36 px | **68 px** |
| `card-title-sm` | 32 px | **60 px** |
| `roadmap-title` | 32 px | **60 px** |
| `pillar-title` | 28 px | **52 px** |
| `inline-stat` | 28 px | **52 px** |
| `close-sub` | 18 px | **34 px** |
| `body` | 16 px | **30 px** |
| `tag-body` | 16 px | **30 px** |
| `red-cite` | 16 px | **30 px** |
| `body-card` | 14 px | **26 px** |
| `hero-meta` | 14 px | **26 px** |
| `body-cell` | 13 px | **24 px** |
| `body-small` | 13 px | **24 px** |
| `tag-label` | 12 px | **22 px** |
| `bullet-body` | 12 px | **22 px** |
| `link` | 12 px | **22 px** |
| `counter` | 11 px | **21 px** |
| `label` | 10 px | **19 px** |
| `label-red` | 10 px | **19 px** |
| `fc-micro` | 10 px | **19 px** |
| `rm-label` | 9 px | **17 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.875.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.
...
⚠️ Even re-anchored, these roles land under 24 px: tag-label, bullet-body, link, counter, label, label-red, fc-micro, rm-label. Chrome only — never reading text.
...
### Canvas System
The system targets `1920px × 1080px` per slide. Slides are absolutely positioned and animate in via opacity + translateY (30px) + scale (0.98 → 1) on 550ms ease. Only one slide is `.active` at a time. Default slide padding is `48px 56px` with major variations:
- Hero slides: `54px 134.4px` top-padding, alignment shifted to top-left.
...
- **The hero tagline on slide-hero is absolutely positioned at `bottom: 86.4px; right: 134.4px`** with a 300px max-width. On very wide aspect ratios or very narrow ones it may misalign with the hero title stack.
- **The progress bar is a 5px height strip** that may visually overlap content on slides with bottom-edge text — the slide-close surface in particular places close-links at `bottom: 54px` which clears the progress bar but only by a small margin.
