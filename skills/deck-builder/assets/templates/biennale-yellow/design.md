...
spacing:
  ...
  gap-strand: "19.4px"
  gap-footer-col: "44px"
  pagenum-bottom: "25.9px"
  pagenum-right: "46.1px"
...
components:
  ...
  strand-row:
    layout: "grid 56px 1fr, gap 24px, border-bottom hairline-soft, padding-bottom 17.3px"
  ...
  ledger-row:
    layout: "grid 92px 1.6fr 0.9fr 80px, gap 26.9px, border-bottom hairline-soft, padding 14px 0"
  ...
  footer-band:
    layout: "grid 4-column with hairline-rule top border on each cell, gap 44px"
...
|---|---|---|
| `numeral-jumbo` | 537.6 px | **538 px** — unchanged |
| `display` | 237.6 px | **238 px** — unchanged |
| `numeral-lg` | 237.6 px | **238 px** — unchanged |
| `display-sm` | 194.4 px | **364 px** |
| `display-md` | 172.8 px | **324 px** |
| `numeral-md` | 129.6 px | **243 px** |
| `display-it` | 118.8 px | **223 px** |
| `date-rail` | 96 px | **180 px** |
| `headline` | 75.6 px | **142 px** |
| `headline-sm` | 56 px | **105 px** |
| `strand-num` | 38 px | **71 px** |
| `strand-title` | 32 px | **60 px** |
| `ledger-title` | 30 px | **56 px** |
| `body-lede` | 18 px | **34 px** |
| `body` | 16 px | **30 px** |
| `mono-date` | 16 px | **30 px** |
| `micro-label` | 14 px | **26 px** |
| `mono-data` | 14 px | **26 px** |
| `body-sm` | 13 px | **24 px** |
| `rail-label` | 13 px | **24 px** |
| `pagenum` | 13 px | **24 px** |
| `micro-label-tight` | 12 px | **22 px** |
...
⚠️ Even re-anchored, these roles land under 24 px: micro-label-tight. Chrome only — never reading text.
...
### Canvas System
The system targets `1920px × 1080px` per slide. Slides are absolutely positioned and crossfade via opacity transition (280ms ease). Only one slide is `.active` at a time. Every measurement uses CSS `clamp()` with both vw and vh terms — typography clamps include `min(...vw, ...vh)` patterns so display type scales by whichever viewport axis is shorter.
...
This system is built as a **viewport-fluid 1920px × 1080px deck** with no responsive breakpoints. Every font size, padding value, and gap uses CSS `clamp()` with both vw and vh terms — `75.6px` is a typical pattern. Display type scales by whichever viewport axis is shorter, so portrait orientations don't blow out the headlines.
