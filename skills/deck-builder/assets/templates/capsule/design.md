...
spacing:
  ...
  pill-pad-sm: "6.4px 19.2px"
  pill-pad-xs: "5.6px 16px"
...
This model was authored fluid for a ~1178 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.63** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.

| role | model (frozen) | **on canvas — use this** |
|---|---|---|
| `display` | 112 px | **183 px** |
| `closing-display` | 80 px | **130 px** |
| `headline` | 56 px | **91 px** |
| `section-headline` | 48 px | **78 px** |
| `quote-display` | 48 px | **78 px** |
| `stat-number` | 48 px | **78 px** |
| `orbit-numeral` | 40 px | **65 px** |
| `card-headline` | 24 px | **39 px** |
| `body` | 18.4 px | **30 px** |
| `subtitle` | 17.6 px | **29 px** |
| `body-sm` | 14.4 px | **23 px** |
| `pill-text-md` | 13.6 px | **22 px** |
| `label` | 12 px | **20 px** |
| `pill-text-sm` | 11.2 px | **18 px** |
| `mini-label` | 10.4 px | **17 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.63.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.
...
⚠️ Even re-anchored, these roles land under 24 px: body-sm, pill-text-md, label, pill-text-sm, mini-label. Chrome only — never reading text.
...
|---|---|---|---|---|
| `{typography.display}` | 112px | Bodoni Moda | 800 | Largest cover/title display |
| `{typography.closing-display}` | 80px | Bodoni Moda | 800 | Conclusive declarative headline |
| `{typography.headline}` | 56px | Bodoni Moda | 700 | Primary slide headline on split or two-column layouts |
| `{typography.section-headline}` | 48px | Bodoni Moda | 700 | Section-opening or centered headline above cards/charts |
| `{typography.quote-display}` | 48px | Bodoni Moda | 600 | Pull-quote body |
| `{typography.card-headline}` | 24px | Bodoni Moda | 700 | Card or pillar-block title |
| `{typography.stat-number}` | 48px | Bodoni Moda | 800 | Large numerical stat figure |
| `{typography.orbit-numeral}` | 40px | Bodoni Moda | 700 | Centered ordinal inside the orbit-center circle |
| `{typography.body}` | 18.4px | Space Grotesk | 400 | Paragraph body |
| `{typography.body-sm}` | 14.4px | Space Grotesk | 400 | Compact body inside a card |
| `{typography.subtitle}` | 17.6px | Space Grotesk | 400 | Uppercase tracked subtitle below a display headline |
| `{typography.pill-text-md}` | 13.6px | Space Grotesk | 600 | Text inside title/closing pills |
| `{typography.pill-text-sm}` | 11.2px | Space Grotesk | 600 | Text inside small floating decorative pills |
| `{typography.label}` | 12px | Space Grotesk | 500 | Header tag pills, attribution lines |
| `{typography.mini-label}` | 10.4px | Space Grotesk | 500 | Mini-pill chips in a tag cluster |

### Defaults
- **Default size for the primary section headline**: `{typography.section-headline}` (clamp 1.8–48px). For two-column or split layouts, `{typography.headline}` (clamp 2–56px).
- **Default size for a cover or opening display moment**: `{typography.display}` (clamp 3–112px).
- **Default size for paragraph body**: `{typography.body}` (clamp 0.95–18.4px).
...
- **Default size for a hero stat figure**: `{typography.stat-number}` (clamp 2–48px).
...
| `{spacing.pill-pad-sm}` | 6.4px 19.2px | Header-tag pill padding |
| `{spacing.pill-pad-xs}` | 5.6px 16px | Mini-pill chip padding |
...
| 32px (32px) | Larger cards: pillar-cards, stat-pills, chart container, visual frame |
| 24px (24px) | Inner dashed frame inside the visual frame |
...
**Decorative floating pill** — A small (60–160px wide × 35–90px tall) pill or circle in a candy fill, tilted at -20° to +25° rotation, positioned absolutely on the slide background. Contains a single uppercase Space-Grotesk word at 0.55–13.6px. Functions as typographic confetti / atmospheric wallpaper. Five to eight per declarative slide.

**Header tag pill** — A small (≈11.2px text) pill in a candy fill (typically lavender) with `pill-pad-sm` padding, centered above a section headline. The system's section-tag chip.
...
### Scaling Behavior
- Display headline scales from 48px at minimum viewport to 112px at maximum.
- Body text scales from 15.2px to 18.4px.
