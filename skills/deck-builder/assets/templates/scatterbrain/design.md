...
spacing:
  slide-pad: 48px
  post-it-pad-lg: "48px 64px"
  post-it-pad-md: "40px"
  post-it-pad-sm: "24px"
  post-it-pad-statement: "56px 64px"
  gap-lg: "48px"
  gap-md: "40px"
  gap-sm: "32px"
...
components:
  ...
  photo-frame:
    ...
    rotation: "±1° to ±2°"
    description: "Polaroid-style image frame. White paper with 16px padding around the inner image area; same drop shadow as post-its; small rotation. The inner image area has a 4:3 aspect ratio."
  chart-canvas:
    background: "#fff"
    padding: "40px"
    boxShadow: "2px 3px 15px {colors.shadow}"
...
This model was authored fluid for a ~1280 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.5** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.

| role | model (frozen) | **on canvas — use this** |
|---|---|---|
| `display-hero` | 72 px | **108 px** |
| `statement` | 56 px | **84 px** |
| `headline` | 48 px | **72 px** |
| `handwritten-lg` | 32 px | **48 px** |
| `title` | 28.8 px | **43 px** |
| `stat-value` | 28.8 px | **43 px** |
| `handwritten` | 25.6 px | **38 px** |
| `handwritten-sm` | 22.4 px | **34 px** |
| `caption-subtitle` | 20.8 px | **31 px** |
| `body` | 20 px | **30 px** |
| `list-item` | 17.6 px | **26 px** |
| `label-script` | 14.4 px | **22 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.5.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.
...
⚠️ Even re-anchored, these roles land under 24 px: label-script. Chrome only — never reading text.
...
|---|---|---|---|---|
| `{typography.display-hero}` | 2.5–72px | Shrikhand | 400 | Cover or closing oversized headline |
| `{typography.statement}` | 2–56px | Shrikhand | 400 | Centered statement or pulled quote |
| `{typography.headline}` | 1.8–48px | Shrikhand | 400 | Primary slide headline / section headline |
| `{typography.title}` | 1.3–28.8px | Shrikhand | 400 | Sub-region or card title |
| `{typography.caption-subtitle}` | 20.8px | Zilla Slab | 400 | Slide subtitle below a hero headline |
| `{typography.body}` | 1–20px | Zilla Slab | 400 | Standard paragraph body |
| `{typography.list-item}` | 17.6px | Zilla Slab | 400 | Bulleted / check-marked list row |
| `{typography.handwritten}` | 1.2–25.6px | Caveat | 400 | Decorative quip, personal note |
| `{typography.handwritten-lg}` | 1.4–32px | Caveat | 600 | Larger handwritten subtitle |
| `{typography.handwritten-sm}` | 1.2–22.4px | Caveat | 500 | Small accent label |
| `{typography.label-script}` | 14.4px | Caveat | 400 | Tracked-caps eyebrow label |
| `{typography.stat-value}` | 28.8px | Shrikhand | 400 | Numerical value in a stat-row |

### Defaults
- **Default size for a primary section headline (inside a post-it)**: `{typography.headline}` (1.8–48px clamp) in Shrikhand.
- **Default size for a cover or closing oversized headline**: `{typography.display-hero}` (2.5–72px clamp).
- **Default size for a centered manifesto / pulled-quote statement**: `{typography.statement}` (2–56px clamp).
- **Default size for paragraph body**: `{typography.body}` (1–20px clamp) in Zilla Slab 400.
- **Default size for list rows (bulleted, check-marked, or compare-list)**: `{typography.list-item}` (17.6px) in Zilla Slab 400.
- **Default size for a side note or quip**: `{typography.handwritten}` (1.2–25.6px clamp) in Caveat 400.
- **Default size for an eyebrow label above a card headline**: `{typography.label-script}` (14.4px) in Caveat, uppercase, 0.15em tracking.
- **Default size for a stat value**: `{typography.stat-value}` (28.8px) in Shrikhand.
...
- **Every display headline is set in Shrikhand.** Substituting Zilla Slab or another display face loses the workshop voice. Even small titles (20.8px) inside feature cards use Shrikhand.
...
### Canvas System
The system targets `1920px × 1080px`. Each `.slide` is absolutely positioned to fill the viewport with `display: none` by default; the `.active` slide is `display: flex` centered. Navigation is JS-driven via arrow keys, space, PageUp/Down, Home/End, touch swipe, and mouse wheel (with 700ms lock to prevent multi-skip).
...
| Token | Value | Use |
|---|---|---|
| `{spacing.slide-pad}` | 48px | Slide outer padding |
| `{spacing.post-it-pad-lg}` | 48px 64px | Hero / title post-it interior padding |
| `{spacing.post-it-pad-statement}` | 56px 64px | Statement post-it interior padding |
| `{spacing.post-it-pad-md}` | 40px | Standard post-it interior padding |
| `{spacing.post-it-pad-sm}` | 24px | Small accent post-it interior padding |
| `{spacing.gap-lg}` | 48px | Multi-column grid gap |
| `{spacing.gap-md}` | 40px | Feature grid gap |
| `{spacing.gap-sm}` | 32px | Timeline row gap |
...
**Photo frame** (`{components.photo-frame}`) — Polaroid-style white card with 16px padding around a 4:3 inner image area. Same drop shadow as post-its; small rotation.
...
The system targets `1920px × 1080px` and uses `clamp()` throughout for fluid scaling. A single media query at `max-width: 900px` reflows multi-column grids and timelines to single-column, neutralizes some rotations (versus-circle becomes inline rather than absolute), and stacks compare-cards vertically.

### Scaling Behavior
- Display headlines scale via `72px` patterns — fluid between minimum and maximum.
- Body text scales from 16px at minimum to 20px at maximum.
...
### Print Behavior
A `@media print` rule sets `page-break-after: always` on each slide with `min-height: 1080px`. Printing produces a sequential page-per-slide output.
...
7. Headlines use Shrikhand at sizes from `{typography.title}` (20.8px) for small card titles up to `{typography.display-hero}` (72px) for cover/closing. Body uses Zilla Slab. Personal notes use Caveat.
