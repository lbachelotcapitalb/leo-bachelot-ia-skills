...
|---|---|---|
| `pixel-hero` | 128 px | **209 px** |
| `display` | 64 px | **104 px** |
| `stat-number` | 56 px | **91 px** |
| `headline` | 45 px | **73 px** |
| `quote-body` | 25.6 px | **42 px** |
| `subhead` | 24 px | **39 px** |
| `hero-tagline` | 19.2 px | **31 px** |
| `body` | 18.4 px | **30 px** |
| `label-eyebrow` | 13.6 px | **22 px** |
| `counter` | 12.8 px | **21 px** |
| `label-pill` | 12 px | **20 px** |
| `chart-value` | 12 px | **20 px** |
| `badge` | 11.2 px | **18 px** |
| `chart-label` | 11.2 px | **18 px** |
| `date-chip` | 11.2 px | **18 px** |
...
⚠️ Even re-anchored, these roles land under 24 px: label-eyebrow, counter, label-pill, chart-value, badge, chart-label, date-chip. Chrome only — never reading text.
...
### Canvas System
The system targets `1920px × 1080px` per slide. Slides are stacked vertically inside a `slides-container` and translated via `transform: translateY(...)` for navigation. Only one slide occupies the viewport at a time, with cubic-bezier `(0.22, 1, 0.36, 1)` easing on an 800ms transition.

Default slide padding is `32.4px 76.8px`. The inner `slide-content` caps at `max-width: 1200px` and centers, so on wide displays content keeps editorial column width while the colored background fills the viewport.
