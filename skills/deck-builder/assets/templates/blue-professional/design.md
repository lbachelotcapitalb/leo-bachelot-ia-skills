...
spacing:
  pad-slide-x: "76.8px"
  pad-slide-y-top: "67.2px"
  pad-slide-y-bottom: "91.8px"
  pad-card-lg: "24px 25.6px"
  pad-card-md: "22.4px 24px"
  pad-card-sm: "16px 19.2px"
  pad-mini: "14.4px 16px"
  gap-grid-lg: "56px"
  gap-grid-md: "32px 48px"
  gap-grid-sm: "24px"
  gap-cards: "19.2px"
  gap-mini: "16px"
  header-margin: "27px"
...
This model was authored fluid for a ~1075 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.786** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.

| role | model (frozen) | **on canvas — use this** |
|---|---|---|
| `quote-mark` | 128 px | **229 px** |
| `h1` | 67.2 px | **120 px** |
| `metric-value` | 48 px | **86 px** |
| `h2` | 41.6 px | **74 px** |
| `blockquote` | 38.4 px | **69 px** |
| `stat-num` | 33.6 px | **60 px** |
| `agenda-num` | 28.8 px | **51 px** |
| `h3` | 24 px | **43 px** |
| `split-highlight` | 24 px | **43 px** |
| `step-circle-text` | 20.8 px | **37 px** |
| `step-title` | 18.4 px | **33 px** |
| `metric-label` | 17.6 px | **31 px** |
| `body` | 16.8 px | **30 px** |
| `h4-eyebrow` | 16 px | **29 px** |
| `bar-label` | 16 px | **29 px** |
| `stat-name` | 15.2 px | **27 px** |
| `bar-pct` | 15.2 px | **27 px** |
| `metric-desc` | 14.4 px | **26 px** |
| `metric-support` | 13.6 px | **24 px** |
| `counter` | 12.8 px | **23 px** |
| `meta` | 12.8 px | **23 px** |
| `insight-num` | 12.5 px | **22 px** |
| `cite` | 12.5 px | **22 px** |
| `stat-context` | 12 px | **21 px** |
| `tag` | 12 px | **21 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.786.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.
...
⚠️ Even re-anchored, these roles land under 24 px: counter, meta, insight-num, cite, stat-context, tag. Chrome only — never reading text.
...
- **Every CTA is the `{components.cta-button}` pattern: solid cobalt pill with cream text, 100px border-radius, 14.4px × 35.2px padding.** CTAs in any other shape or color don't exist.
- **Every tag pill is the `{components.tag-pill}` pattern: soft cobalt tint with cobalt text, 100px border-radius, 5.6px × 14.4px padding.** Tag pills with solid backgrounds or borders break the system.
...
### Canvas System
The system targets `1920px × 1080px` per slide. Slides are absolutely positioned and animated in via opacity + translateX (40px → 0px) on 500ms ease. Only the `.active` slide is `opacity: 1`; the previous slide gets a `.prev` class and translates -40px out of view.

Default slide padding is asymmetric: `67.2px` left, top, and right; `91.8px` bottom. The extra bottom space reserves room for the fixed slide-counter (left) and nav-controls (right) which sit at `bottom: 27px` overlapping the viewport bottom.

### Padding and Gap Scale

| Token | Value | Use |
|---|---|---|
| `{spacing.pad-slide-x}` | 76.8px | Slide horizontal padding |
| `{spacing.pad-slide-y-top}` | 67.2px | Slide top padding |
| `{spacing.pad-slide-y-bottom}` | 91.8px | Slide bottom padding (extra to clear nav chrome) |
| `{spacing.pad-card-lg}` | 24px 25.6px | Large card internal padding (metric cards) |
| `{spacing.pad-card-md}` | 22.4px 24px | Medium card internal padding (stat cells) |
| `{spacing.pad-card-sm}` | 16px 19.2px | Small card internal padding (detail blocks) |
| `{spacing.pad-mini}` | 14.4px 16px | Mini card internal padding (mini-stat cells) |
| `{spacing.gap-grid-lg}` | 56px | Large gap between split columns |
| `{spacing.gap-grid-md}` | 32px 48px | Medium gap in two-column grids (row × col) |
| `{spacing.gap-grid-sm}` | 24px | Standard gap in 3-column metric grids |
| `{spacing.gap-cards}` | 19.2px | Gap between dashboard stat cells |
| `{spacing.gap-mini}` | 16px | Gap between mini-stats |
| `{spacing.header-margin}` | 27px | Margin below the slide-header |

### Persistent Chrome
Three elements appear on every slide:
- **Slide counter** at bottom-left — Space Grotesk 12.8px weight 500 in text-muted gray, fixed at `bottom: 27px; left: 57.6px`.
- **Nav controls** at bottom-right — two circular 44px nav-buttons with 1.5px cobalt-at-20% borders, fixed at `bottom: 27px; right: 57.6px`. Disabled state at 30% opacity (first slide / last slide).
...
Blue Professional is designed as a **1920×1080 presentation system** (effective 1920px × 1080px). Sizing uses CSS `clamp()` for type and `vw / vh / rem` for spacing. The system has minimal responsive behavior — it's built for landscape presentation contexts.
...
- Slide padding reduces from 67.2px / 91.8px to 27px / 57.6px.
- Agenda grid gap reduces.
- Metric and detail card padding reduces to 16px.
