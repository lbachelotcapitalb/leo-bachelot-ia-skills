...
components:
  ...
  vertical-line:
    ...
    background: "{colors.line}"
    opacity: 0.3
    description: "Optional decorative vertical hairline anchored at left edge (default 153.6px from edge) at low opacity. Provides drafting-paper grid feel."
  horizontal-accent:
    width: "384px"
    height: "1px"
  ...
  geo-decoration:
    border: "1px solid {colors.line}"
    borderRadius: "50%"
    description: "Large decorative ring (30–960px diameter) anchored to a corner or center. Always pairs with a ::before pseudo-element rendering a smaller dashed ring inside it, suggesting concentric drafting."
  geo-ring:
    width: "960px"
    height: "960px"
    border: "1px solid {colors.line}"
  ...
  team-photo:
    ...
    border: "1px solid {colors.line}"
    background: "{colors.bg-secondary}"
    description: "Circular portrait frame (230.4px) in slightly darker stone, ringed in taupe, holding a single Playfair initial in accent taupe at center."
  nav-arrow:
    width: 40px
    height: 40px
    border: "1px solid {colors.line}"
...
This model was authored fluid for a ~1126 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.705** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.

| role | model (frozen) | **on canvas — use this** |
|---|---|---|
| `display` | 88 px | **150 px** |
| `quote-mark` | 80 px | **136 px** |
| `h1` | 72 px | **123 px** |
| `h2` | 48 px | **82 px** |
| `stat-figure` | 32 px | **55 px** |
| `team-initial` | 32 px | **55 px** |
| `h3` | 25.6 px | **44 px** |
| `agenda-numeral` | 24 px | **41 px** |
| `card-headline` | 20.8 px | **35 px** |
| `subtitle` | 20.8 px | **35 px** |
| `timeline-headline` | 19.2 px | **33 px** |
| `body` | 17.6 px | **30 px** |
| `body-sm` | 14.4 px | **25 px** |
| `attribution` | 13.6 px | **23 px** |
| `label` | 12 px | **20 px** |
| `micro` | 11.2 px | **19 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.705.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.
...
⚠️ Even re-anchored, these roles land under 24 px: attribution, label, micro. Chrome only — never reading text.
...
**Density philosophy: sparse and breathing.** Cartesian reads as elegant when sparse and broken when crowded. A correctly composed slide pairs one Playfair headline with one body paragraph, or one chart with one short caption, framed by generous negative space. Most slide composition uses centered or asymmetric two-column layouts with `5–115.2px` gutters and significant vertical breathing room. The decorative compass arcs reinforce the sparseness — they are most effective when there is room around them. A slide that feels broken in this system is one packed edge-to-edge with content; the correct density is "one clear idea, well-framed, on stone paper."
...
|---|---|---|---|---|
| `{typography.display}` | 88px | Playfair Display | 400 | Cover or large opening title |
| `{typography.h1}` | 72px | Playfair Display | 400 | Section-opening or large closing headline |
| `{typography.h2}` | 48px | Playfair Display | 400 | Primary slide headline |
| `{typography.h3}` | 25.6px | Playfair Display | 400 | Sub-headline within a region |
| `{typography.card-headline}` | 20.8px | Playfair Display | 400 | Card or pillar-block title |
| `{typography.timeline-headline}` | 19.2px | Playfair Display | 400 | Timeline-step or phase headline |
| `{typography.stat-figure}` | 32px | Playfair Display | 400 | Numerical stat figure (typically inline in a stats cluster) |
| `{typography.team-initial}` | 32px | Playfair Display | 400 | Initial letter centered inside a team-photo circle |
| `{typography.agenda-numeral}` | 24px | Playfair Display | 400 | Ordinal numeral preceding an agenda row |
| `{typography.quote-mark}` | 80px | Playfair Display | 400 | Opening quotation glyph for a pull-quote |
| `{typography.subtitle}` | 20.8px | Inter | 400 | Subtitle paragraph below a display headline |
| `{typography.body}` | 17.6px | Inter | 400 | Standard paragraph body |
| `{typography.body-sm}` | 14.4px | Inter | 400 | Compact body inside cards or timeline items |
| `{typography.attribution}` | 13.6px | Inter | 400 | Attribution line below a quote, with 2px tracking + uppercase |
| `{typography.label}` | 12px | Inter | 500 | Section label / eyebrow above a headline, with 3px tracking + uppercase |
| `{typography.micro}` | 11.2px | Inter | 400 | Image label, slide counter, smallest chrome text |

### Defaults
- **Default size for the primary slide headline**: `{typography.h2}` (clamp 1.8–48px).
- **Default size for a cover or large opening headline**: `{typography.h1}` (clamp 2.5–72px); for the most expansive cover, `{typography.display}` (clamp 3–88px).
- **Default size for paragraph body**: `{typography.body}` (clamp 0.9–17.6px).
- **Default size for any inline label or eyebrow**: `{typography.label}` (12px) — always 3px tracking + uppercase.
...
- **Default size for a stat numeral inside a stats cluster**: `{typography.stat-figure}` (32px). Cartesian does not have a hero-stat-numeral pattern (no 5–112px display stat); stats are presented as inline ranks at modest scale.
...
### Canvas System
The canvas is `1920px × 1080px` — full viewport with hidden overflow. Each `.slide` is absolutely positioned to fill the viewport; one slide carries `.active` (opacity 1, visibility visible) at a time. Transitions are 0.6s opacity + visibility fades.

### Padding and Gap Scale
| Token | Value | Use |
|---|---|---|
| `{spacing.pad-y}` | 43.2px | Default vertical slide padding |
| `{spacing.pad-x}` | 76.8px | Default horizontal slide padding |
| `{spacing.gap-xl}` | 115.2px | Major two-column gutter (most spacious) |
| `{spacing.gap-lg}` | 96px | Standard two-column or grid gutter |
| `{spacing.gap-md}` | 57.6px | Card-grid gutter |
| `{spacing.gap-sm}` | 21.6px | Agenda row vertical padding, attribution spacing |
| `{spacing.card-pad}` | 43.2px 38.4px | Internal padding for content cards |

Headlines typically carry a 2–32.4px margin-bottom from labels above and a 21.6px margin-bottom from body below. Section headers above 3-card or 4-team grids carry a 64.8px margin-bottom from the grid (significant breathing room).
...
- A large ringed `geo-decoration` anchored to a corner (typically 30–960px diameter) with an inner dashed ring at ~80% diameter (the `::before` pseudo).
- A `vertical-line` at ~153.6px from the left edge running floor-to-ceiling at 30% opacity.
...
- Bottom-right `slide-counter` text in Inter at 12px with 2px tracking in `{colors.accent}`.
...
- **Type contrast** — Playfair serif vs Inter sans; size scale from 80px down to 11.2px.
...
| Value | Use |
|---|---|
| 50% (circle) | Every circular element: `card-icon` (40px), `team-photo` (230.4px), `nav-dot` (8px), `geo-circle`, `geo-arc`, `geo-decoration`, `geo-ring` |
...
**Vertical line** — A single 1px taupe column anchored at ~153.6px from the left slide edge, running floor-to-ceiling at 30% opacity. Suggests a drafting-paper alignment guide. Optional per slide.
...
**Team-photo frame** — A 230.4px circular block in `{colors.bg-secondary}` with a 1px taupe ring, holding a single Playfair-400 initial at 32px in `{colors.accent}` centered. Used as portrait placeholder.
...
**Agenda row** — A flex row containing a Playfair `{typography.agenda-numeral}` in `{colors.accent}` at left, then a label at right, with 21.6px padding and a 1px taupe bottom border. Stacked rows form an agenda list.
...
**Quote mark** — A 80px Playfair quote glyph at 50% opacity in taupe, positioned above a Playfair `{typography.h2}` headline + a small uppercase taupe attribution below.
...
### Scaling Behavior
- Display headlines scale from 40px at minimum viewport to 88px at maximum via `clamp()`.
- Body text scales from 14.4px to 17.6px.
...
### Print / Export
Not explicitly handled. Each slide is a 1920px × 1080px block; export workflows should snapshot each slide at 1920×1080.
...
- The `vertical-line` and `horizontal-accent` decorative elements have hardcoded position values (153.6px from left, 162px from bottom); using them off-default-position requires per-instance style overrides.
