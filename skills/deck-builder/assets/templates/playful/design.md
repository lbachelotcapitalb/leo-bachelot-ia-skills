...
spacing:
  pad-slide-lg: "64px 80px"
  pad-slide-md: "48px 64px"
  pad-card-lg: "32px 48px"
  pad-card-md: "24px"
  gap-lg: "48px"
  gap-md: "32px"
  gap-sm: "24px"
...
|---|---|---|
| `display-hero` | 144 px | **246 px** |
| `display` | 112 px | **191 px** |
| `number-hero` | 112 px | **191 px** |
| `headline` | 80 px | **136 px** |
| `statement` | 72 px | **123 px** |
| `title` | 56 px | **95 px** |
| `number-md` | 40 px | **68 px** |
| `number-sm` | 32 px | **55 px** |
| `title-sm` | 20.8 px | **35 px** |
| `body-md` | 19.2 px | **33 px** |
| `body` | 17.6 px | **30 px** |
| `label-eyebrow` | 13.6 px | **23 px** |
| `caption` | 13.6 px | **23 px** |
| `tag` | 12 px | **20 px** |
...
⚠️ Even re-anchored, these roles land under 24 px: label-eyebrow, caption, tag. Chrome only — never reading text.
...
|---|---|---|---|---|
| `{typography.display-hero}` | 4–144px | Syne | 800 | Oversized cover or opening date/title |
| `{typography.display}` | 3–112px | Syne | 800 | Closing or major declarative headline |
| `{typography.headline}` | 2.5–80px | Syne | 700 | Primary section headline |
| `{typography.statement}` | 2.5–72px | Syne | 700 | Long-form quoted statement or manifesto line |
| `{typography.title}` | 2–56px | Syne | 700 | Region or section title |
| `{typography.number-hero}` | 4–112px | Syne | 800 | Hero statistic numeral |
| `{typography.number-md}` | 40px | Syne | 800 | Mid-scale ordinal or stat numeral |
| `{typography.number-sm}` | 32px | Syne | 800 | Inline ordinal or step numeral |
| `{typography.title-sm}` | 20.8px | Syne | 700 | Card title within a small block |
| `{typography.body-md}` | 19.2px | Space Grotesk | 500 | Subtitle or emphasized body |
| `{typography.body}` | 1–17.6px | Space Grotesk | 400 | Paragraph body |
| `{typography.label-eyebrow}` | 13.6px | Space Grotesk | 600 | Section eyebrow above a headline, uppercase tracked |
| `{typography.caption}` | 13.6px | Space Grotesk | 500 | Subtitle, fine print, footnote |
| `{typography.tag}` | 12px | Space Grotesk | 600 | Pill tag text inside a charcoal label |

### Defaults
- **Default size for a primary section headline**: `{typography.headline}` (2.5–80px clamp).
- **Default size for a cover or major opening moment**: `{typography.display-hero}` (4–144px clamp).
- **Default size for paragraph body**: `{typography.body}` (1–17.6px clamp).
- **Default size for a subtitle or emphasized lead body**: `{typography.body-md}` (19.2px).
- **Default size for any inline caption, footnote, or fine print**: `{typography.caption}` (13.6px).
- **Default size for a hero numerical figure**: `{typography.number-hero}` (4–112px clamp).
- **Default size for a stat tile or ordinal numeral**: `{typography.number-md}` (40px).
...
- **Statistics and numerals are always weight 800.** Even mid-scale numerals (32px) follow the display-weight convention. A weight-500 numeral reads as inventory, not a brag.
...
### Canvas System
The system targets `1920px × 1080px` — full viewport. Every `.slide` is absolutely positioned to fill the viewport, and only the `.active` slide is visible (opacity 1, others at opacity 0). Slide navigation is JS-driven via arrow keys, space, click on next/prev buttons, and touch swipe. A 4px-tall charcoal progress bar runs along the bottom of the viewport, growing to indicate slide position. All sizes use `clamp()` so the layout fluidly scales without breakpoints down to a mobile reflow at 768px.

### Padding Scale
| Token | Value | Use |
|---|---|---|
| `{spacing.pad-slide-lg}` | 64px 80px | Cover, statement, and closing slides — generous outer padding |
| `{spacing.pad-slide-md}` | 48px 64px | Standard content slides — outer padding |
| `{spacing.pad-card-lg}` | 32px 48px | Large card or contact block interior padding |
| `{spacing.pad-card-md}` | 24px | Standard card interior padding |
| `{spacing.gap-lg}` | 48px | Large grid/flex gaps between major regions |
| `{spacing.gap-md}` | 32px | Standard grid/flex gaps between cells |
| `{spacing.gap-sm}` | 24px | Tight inline gaps |
...
**Tag pill** — A small charcoal rectangle with peach text at 12px weight 600. Anchored to the bottom-left of an image frame as a category label. The only inverted text-on-charcoal element that is not a full card.

**Vertical spine label** — Syne 700 text at 24px rotated 90deg, anchored to a slide's right edge with 0.1em letter-spacing. Reads as a magazine-spine wayfinder.
...
The system targets `1920px × 1080px` and uses `clamp()` throughout, so type and padding scale fluidly between viewport sizes without media queries. A single media query at `max-width: 768px` reflows multi-column grids to single-column, collapses the horizontal timeline track to vertical, and reduces slide padding to 32px.
...
**站酷快乐体 ZCOOL KuaiLe** is a rounded, friendly Chinese display face with quirky humanist proportions — the closest CDN-available equivalent to Syne's expressive studio voice. At display weights (4–144px), KuaiLe reads as a contemporary independent Chinese studio voice, not a corporate or formal one. It pairs naturally with the peach canvas and charcoal ink. **Drop the negative letter-spacing (-0.01em to -0.03em) for Chinese display** — KuaiLe is designed on a square em-box and tightening causes glyph collision. The face's intrinsic warmth replaces what negative tracking does for Syne.

**悠哉字体 Yozai** is a Chinese sans-serif designed for relaxed body reading — slightly rounded terminals, friendly humanist proportions, designed to match the warmth that Space Grotesk brings to Latin. Set every Chinese body paragraph and label in Yozai 400. The eyebrow label treatment (0.15em letter-spacing + uppercase in Latin) does not transfer to CJK; for Chinese eyebrows, use Yozai at the same 13.6px size with letter-spacing 0 and no uppercase — small size alone reads as label.
...
4. Any new body or label uses Space Grotesk at weight 400–600. Use `{typography.label-eyebrow}` (13.6px, weight 600, uppercase, 0.15em tracking) for the small label above a headline.
5. Any new statistic or numeral uses Syne at weight 800 with negative letter-spacing. Even small numerals (32px) follow the display-weight convention.
