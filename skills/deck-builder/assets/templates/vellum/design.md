...
components:
  ...
  pin-stat-val:
    ...
    color: "{colors.yellow}"
    description: "Large italic serif stat numeral (105.6px italic Cormorant Garamond), centered in a pin-stat tile."
  pin-stat-label:
  ...
  quote-mark:
    ...
    color: "{colors.teal}"
    description: "A 134.4px italic Cormorant Garamond opening quote glyph in dusty teal, sitting centered above a centered pull-quote. The teal color is the system's only large-graphic accent."
---
...
|---|---|---|
| `display` | 211.2 px | **314 px** |
| `h1` | 134.4 px | **200 px** |
| `quote-mark` | 134.4 px | **200 px** |
| `stat-value` | 105.6 px | **157 px** |
| `h2` | 76.8 px | **114 px** |
| `quote-text` | 61.4 px | **91 px** |
| `h3` | 46.1 px | **68 px** |
| `lead` | 28.8 px | **43 px** |
| `pin-note` | 22.1 px | **33 px** |
| `bar-val` | 21.1 px | **31 px** |
| `body` | 20.2 px | **30 px** |
| `caption` | 16.3 px | **24 px** |
| `label` | 13.8 px | **20 px** |
...
⚠️ Even re-anchored, these roles land under 24 px: label. Chrome only — never reading text.
...
|---|---|---|---|---|---|
| `{typography.display}` | 211.2px | Cormorant Garamond | 400 | italic | Cover hero at maximum scale |
| `{typography.h1}` | 134.4px | Cormorant Garamond | 400 | italic | Chapter or statement headline |
| `{typography.quote-mark}` | 134.4px | Cormorant Garamond | 400 | italic | Decorative opening quote glyph (in teal) |
| `{typography.stat-value}` | 105.6px | Cormorant Garamond | 400 | italic | Large italic serif stat numeral |
| `{typography.h2}` | 76.8px | Cormorant Garamond | 400 | italic | Primary slide headline |
| `{typography.quote-text}` | 61.4px | Cormorant Garamond | 400 | italic | Pull-quote body |
| `{typography.h3}` | 46.1px | Cormorant Garamond | 500 | italic | Sub-headline, compare panel title |
| `{typography.lead}` | 28.8px | DM Sans | 400 | upright | Lead paragraph, intro sentence |
| `{typography.pin-note}` | 22.1px | Courier Prime | 500 | upright | Pin-annotation text (in teal), bar-val accent |
| `{typography.bar-val}` | 21.1px | Courier Prime | 400 | upright | Chart bar value label |
| `{typography.body}` | 20.2px | DM Sans | 400 | upright | Default body paragraph |
| `{typography.caption}` | 16.3px | DM Sans | 400 | upright | Caption, secondary text |
| `{typography.label}` | 13.8px | Courier Prime | 400 | upright | Chrome label, slide counter, kicker, stat-label |

### Defaults

- **Default size for a primary section headline**: `{typography.h2}` (76.8px).
- **Default size for a chapter or statement headline**: `{typography.h1}` (134.4px).
- **Default size for a cover hero**: `{typography.display}` (211.2px).
- **Default size for body paragraph**: `{typography.body}` (20.2px).
- **Default size for a lead sentence**: `{typography.lead}` (28.8px).
- **Default size for any chrome label, kicker, or mono metadata**: `{typography.label}` (13.8px).
- **Default size for the pin-note**: `{typography.pin-note}` (22.1px).
- **Default size for a statistical numeral**: `{typography.stat-value}` (105.6px italic).
...
- **The large quote-mark glyph (134.4px italic Cormorant Garamond) renders in `{colors.teal}`, not yellow.** The teal quote mark is the only place a large-scale graphic is colored teal; everywhere else teal appears in small text (label-size).
...
Vellum targets `1920px × 1080px` — full viewport. Each `.slide` flexes to fill the viewport exactly, and slides sit side-by-side in a horizontal strip. All sizes use viewport-relative units (`vw`, `vh`) so layout scales fluidly.

### Padding and Gap Scale

| Token | Value | Use |
|---|---|---|
| `{spacing.pad-x}` | 115.2px | Horizontal slide padding |
| `{spacing.pad-y}` | 64.8px | Vertical slide padding |
| `{spacing.gap-lg}` | 54px | Between major content sections |
| `{spacing.gap-md}` | 32.4px | Between related elements |
| `{spacing.gap-sm}` | 16.2px | Between tightly coupled elements |
...
**Italic display headline** — Cormorant Garamond italic at weight 400, 4–211.2px, centered on the slide in `{colors.yellow}`. The headline is the dominant visual element of every content slide.

**Pin-annotation** — a stack of 1–3 Courier Prime mono lines in `{colors.teal}`, positioned absolute at the bottom-left corner of every slide. Lines typically include a slide counter (e.g., "04 / 09"), a short typed label, and an optional second pin phrase. Lines stack with 3.2px gap, max-width 422.4px. This is the system's signature.

**Teal quote-mark glyph** — a 134.4px italic Cormorant Garamond `"` in `{colors.teal}`, centered above a centered pull-quote. The only large-scale teal element in the system.

**Kicker** — Courier Prime mono at label size (13.8px) in `{colors.teal}`, with 0.1em letter-spacing. Sits above a headline.
...
**Pin-stat** — a centered column containing an italic serif stat numeral (105.6px) above a mono caption in muted yellow. In a row, pin-stats are separated by single 1px hairlines at 20% yellow opacity; the last stat in row drops the border.
...
**Image placeholder** — a 1px-dashed rectangle in `{colors.border}` with a faint navy interior, centered Courier Prime mono label, minimum 302.4px height. The dashed border is the "slot" indication.
...
- Place a pin-annotation in the bottom-left corner of every slide — Courier Prime mono in `{colors.teal}`, 22.1px, max-width 422.4px. The pin-annotation is the system's persistent signature.
...
- Pad slides with `{spacing.pad-x}` 115.2px and `{spacing.pad-y}` 64.8px, increasing 1.2–1.4× on quote slides.
...
- Display headline scales: 211.2px → ~211px at 1920px viewport, ~141px at 1280px.
- Body text scales: 20.2px → ~20px at 1920px, ~13px at 1280px.
...
| Quote-mark glyph (134.4px, teal) | Cormorant Garamond italic 400 | LXGW WenKai 400 or `「`/`『` in NSC | 400 | For Chinese quotes, replace `"` with `「` (full-width corner bracket) in LXGW WenKai or NSC at the same teal color. |
...
**Warning — baseline mismatch at display sizes.** Cormorant Garamond italic's optical center sits below LXGW WenKai's optical center, especially at 211.2px display and 134.4px h1. A phrase like `Vellum 羊皮纸` will show the Chinese characters floating slightly high relative to the italic Latin baseline. Mitigations:
...
- **Line-height**: body 1.75–1.85 (Vellum's 1.65 DM Sans-default is close, but bump for CJK strokes); display 1.15–1.25 (looser than Latin 0.92–1.05 because CJK glyphs at 7–211.2px need vertical breathing).
...
LXGW WenKai is an excellent match for Vellum's gallery-essay register. The kaiti's hand-written warmth is closely related to Cormorant Garamond italic's intimate personal voice — both faces read as personal, considered, slightly intimate. Against the deep periwinkle field in warm chartreuse-yellow, LXGW WenKai at 7–211.2px display reads as **handwritten chartreuse on a gallery wall** — arguably more poetic in Chinese than the original Cormorant treatment in English. The kaiti's stroke modulation pairs naturally with the yellow's warmth.
...
- LXGW WenKai has only a single weight (regular). The system's h3 (weight 500 italic) cannot step up in weight in CJK — the visual hierarchy step must come from size alone (h2 76.8px → h3 46.1px). For decks that need stronger sub-headline emphasis, consider NSC 500 as an h3 alternate.
...
- The teal quote-mark glyph (`"` at 134.4px in `{colors.teal}`) does not exist in CJK conventions. Use `「` or `『` (full-width corner bracket) at the same 134.4px size in teal — this is the correct Chinese equivalent and preserves the system's "teal quote moment" signal.
