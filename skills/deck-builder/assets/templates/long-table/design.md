...
spacing:
  ...
  slide-pad-h-wide: "134.4px"
  slide-pad-h-narrow: "230.4px"
  slide-pad-top-default: "108px"
  slide-pad-top-cover: "64.8px"
  slide-pad-bottom-default: "118.8px"
  slide-pad-bottom-wide: "151.2px"
  gap-section: "32.4px"
  gap-content: "21.6px"
  gap-row: "17.3px"
  gap-tight: "13px"
...
This model was authored fluid for a ~1088 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.765** re-anchors the ladder on the 1920×1080 canvas, anchored on `body-roman` → 30 px. Ratios are preserved, so the model's hierarchy is intact.
...
|---|---|---|
| `display-jumbo-numeral` | 410.4 px | **410 px** — unchanged |
| `display-cover` | 162 px | **286 px** |
| `display` | 140.4 px | **248 px** |
| `headline-xl` | 113.4 px | **200 px** |
| `headline` | 108 px | **191 px** |
| `headline-md` | 90.7 px | **160 px** |
| `quote` | 79.9 px | **141 px** |
| `card-title` | 44 px | **78 px** |
| `edition-label` | 30 px | **53 px** |
| `course-name` | 28 px | **49 px** |
| `info-value` | 28 px | **49 px** |
| `body-serif-italic-lg` | 28 px | **49 px** |
| `tagline` | 26 px | **46 px** |
| `body-serif-italic` | 22 px | **39 px** |
| `stats` | 22 px | **39 px** |
| `pill-text` | 20 px | **35 px** |
| `edition-label-tracked` | 18 px | **32 px** |
| `who-tag` | 18 px | **32 px** |
| `body-roman` | 17 px | **30 px** |
| `meta-tag` | 16 px | **28 px** |
| `info-key` | 16 px | **28 px** |
| `pagenum` | 16 px | **28 px** |
| `nav-hint` | 13 px | **23 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.765.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.
...
⚠️ Even re-anchored, these roles land under 24 px: nav-hint. Chrome only — never reading text.
...
### Canvas System
The system targets a fluid viewport — each `.slide` is `1920px × 1080px` with absolute positioning. Slides stack inside a `.stage` container with the paper-texture overlay sitting absolutely on the stage. Only one `.slide.active` is visible at a time; opacity transitions between slides take 280ms.

### Padding Scale (uses `clamp()` ranges)
| Token | Range | Use |
|---|---|---|
| `{spacing.slide-pad-h-default}` | 96px | Default horizontal slide padding |
| `{spacing.slide-pad-h-wide}` | 134.4px | Wider horizontal padding for featured / calendar slides |
| `{spacing.slide-pad-h-narrow}` | 230.4px | Narrow horizontal padding for menu / quote slides (forces content into a column) |
| `{spacing.slide-pad-top-default}` | 108px | Default top padding |
| `{spacing.slide-pad-top-cover}` | 64.8px | Cover top padding (less, to give the title room) |
| `{spacing.slide-pad-bottom-default}` | 118.8px | Default bottom padding (leaves room for page-num chrome) |
| `{spacing.slide-pad-bottom-wide}` | 151.2px | Wider bottom padding for featured / quote slides |
| `{spacing.gap-section}` | 32.4px | Between major content sections |
| `{spacing.gap-content}` | 21.6px | Between related content blocks |
| `{spacing.gap-row}` | 17.3px | Between row-level elements (course rows, ledger rows) |
