...
components:
  ...
  stat-cell:
    ...
    description: "Rule-topped vertical cell with a 105.6px weight-200 numeral, a Jost label, and a mono source note."
  timeline-dot:
    width: 8px
    height: 8px
    borderRadius: 50%
...
This model was authored fluid for a ~1350 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.422** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.

| role | model (frozen) | **on canvas — use this** |
|---|---|---|
| `display` | 163.2 px | **232 px** |
| `stat-value` | 105.6 px | **150 px** |
| `h1` | 96 px | **137 px** |
| `flow-num` | 67.2 px | **96 px** |
| `h2` | 61.4 px | **87 px** |
| `quote-serif` | 61.4 px | **87 px** |
| `insight-serif` | 53.8 px | **77 px** |
| `h3` | 38.4 px | **55 px** |
| `lead` | 28.8 px | **41 px** |
| `body` | 21.1 px | **30 px** |
| `caption` | 16.3 px | **23 px** |
| `label` | 13.8 px | **20 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.422.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.
...
- Generous 153.6px horizontal padding — the most spacious in the template library.
...
|---|---|---|---|---|
| `{typography.display}` | 163.2px | Jost | 200 | Cover or opening hero display — generous and airy |
| `{typography.h1}` | 96px | Jost | 200 | Chapter-opening or section-break headline |
| `{typography.h2}` | 61.4px | Jost | 300 | Primary content-slide headline |
| `{typography.stat-value}` | 105.6px | Jost | 200 | Large numerical figure inside a stat cell |
| `{typography.quote-serif}` | 61.4px | Lora italic | 400 | Pull-quote body text |
| `{typography.insight-serif}` | 53.8px | Lora | 400 | Insight-card title |
| `{typography.h3}` | 38.4px | Jost | 400 | Sub-headline, region heading, flow-step title |
| `{typography.lead}` | 28.8px | Jost | 300 | Lead paragraph or large bullet item |
| `{typography.body}` | 21.1px | Jost | 300 | Body paragraph |
| `{typography.caption}` | 16.3px | Jost | 300 | Image caption, source note, fine print |
| `{typography.label}` | 13.8px | JetBrains Mono | 400 | Kicker, chrome label, axis label, version tag |
| `{typography.flow-num}` | 67.2px | Jost | 200 | Large step numeral in a process diagram |

### Defaults

- **Default primary section headline**: `{typography.h2}` (61.4px at weight 300). Don't reach for `{typography.h1}` for a standard content slide — that size is for chapter breaks.
- **Default opening or cover display**: `{typography.display}` (163.2px at weight 200).
- **Default body paragraph size**: `{typography.body}` (21.1px at weight 300).
- **Default lead paragraph size**: `{typography.lead}` (28.8px at weight 300) when a paragraph is the single supporting block under a headline.
- **Default label / kicker size**: `{typography.label}` (13.8px).
...
The system targets a fluid `1920px × 1080px` viewport with all sizes in `vw`/`vh`. The deck is a horizontal flex strip with slide-to-slide transitions at 0.9s with a smooth easing curve. Animation tokens (`fade-up`, `fade-in`, `reveal-right`, `reveal-left`, `scale-in`) are available with stagger delays via `data-delay` attributes — these run on each slide entrance.

### Padding and Gap Scale

| Token | Value | Use |
|---|---|---|
| `{spacing.pad-x}` | 153.6px | Slide horizontal padding (the most generous in the library) |
| `{spacing.pad-y}` | 64.8px | Slide vertical padding |
| `{spacing.gap-lg}` | 54px | Between major content sections |
| `{spacing.gap-md}` | 32.4px | Between related elements |
| `{spacing.gap-sm}` | 16.2px | Between tightly related elements |

The slide container reserves an additional 67.2px on the left for a vertical sidebar element (currently disabled in the source — see Known Gaps), so total left padding is ~220.8px. Content sits inside this generous gutter system.
...
3. **Generous whitespace** — the primary depth signal is the 153.6px horizontal gutter and the space around each typographic moment.
...
**Insight card** — A 16px-rounded rectangle (`{components.insight-card}`) on a slightly warmer cream surface. Carries a Lora roman title (53.8px weight 400) and a Jost body block (weight 300) pushed to the bottom of the card. The card is identified by its rounded corners and its serif title, not by a color contrast.

**Stat cell** — A vertical region with a 1px solid black top rule (`{components.stat-cell}`), containing a 105.6px Jost-200 numeral, a Jost label, and a small mono source note. Three stat cells side by side is the canonical arrangement.
...
**Pie / donut chart** — A circle (`{components.pie-donut}`) at 453.6px size, with a same-color circular cutout via `::after` to create the donut ring. The chart sits beside a legend column.
...
- Don't crowd a slide. The horizontal padding is 153.6px for a reason — content needs to breathe.
...
The 153.6px horizontal padding (the most generous in the library) was tuned for Latin's narrower glyph widths. Chinese characters are roughly square and consume more horizontal space at the same point size. Long Chinese headlines that would fit on one line in English may wrap to two. Reduce display headline sizes by ~15% (Jost 163.2px → Noto Sans SC 138.2px) when the headline is pure Chinese, or accept the wrap as part of the editorial rhythm.
...
- Lora italic at large sizes (61.4px quote body) has noticeably wider stroke contrast than the surrounding Jost — this is intentional but creates a perceived weight jump. Don't compensate by making the surrounding text heavier.
