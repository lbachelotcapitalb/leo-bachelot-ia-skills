...
components:
  ...
  stat-card:
    ...
    description: "Stat tile with a 2px top rule. Value at 105.6px weight 900 in the surface's foreground color; label and mono note beneath."
  bullet-marker:
    content: "—"
...
This model was authored fluid for a ~1415 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.357** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.

| role | model (frozen) | **on canvas — use this** |
|---|---|---|
| `display` | 230.4 px | **313 px** |
| `h1` | 144 px | **195 px** |
| `stat-value` | 105.6 px | **143 px** |
| `h2` | 92.2 px | **125 px** |
| `quote-text` | 73 px | **99 px** |
| `h3` | 53.8 px | **73 px** |
| `lead` | 30.7 px | **42 px** |
| `body` | 22.1 px | **30 px** |
| `caption` | 16.3 px | **22 px** |
| `label` | 13.8 px | **19 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.357.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.
...
⚠️ Even re-anchored, these roles land under 24 px: caption, label. Chrome only — never reading text.
...
- Headlines run massive — cover display at 230.4px (~230px at 1920px viewport), statements at 144px, section headers at 92.2px.
...
|---|---|---|---|---|
| `{typography.display}` | 230.4px | Barlow | 900 | Cover hero at maximum scale |
| `{typography.h1}` | 144px | Barlow | 900 | Chapter title, full-slide statement headline |
| `{typography.stat-value}` | 105.6px | Barlow | 900 | Statistical numeral inside a stat card |
| `{typography.h2}` | 92.2px | Barlow | 900 | Primary slide headline |
| `{typography.quote-text}` | 73px | Barlow | 900 | Pull-quote body (uppercase, no quote marks) |
| `{typography.h3}` | 53.8px | Barlow | 700 | Sub-headline, panel title |
| `{typography.lead}` | 30.7px | Barlow | 500 | Lead paragraph, intro sentence |
| `{typography.body}` | 22.1px | Barlow | 400 | Default body paragraph, bullet body |
| `{typography.caption}` | 16.3px | Barlow | 400 | Caption, source attribution |
| `{typography.label}` | 13.8px | IBM Plex Mono | 500 | Mono chrome, slide counter, chapter number, stat note, cover-meta |

### Defaults

- **Default size for a primary section headline**: `{typography.h2}` (92.2px).
- **Default size for a chapter or statement headline**: `{typography.h1}` (144px).
- **Default size for a cover hero**: `{typography.display}` (230.4px).
- **Default size for a paragraph body**: `{typography.body}` (22.1px).
- **Default size for a lead sentence**: `{typography.lead}` (30.7px).
- **Default size for any chrome label, counter, or mono caption**: `{typography.label}` (13.8px).
- **Default size for a statistical numeral**: `{typography.stat-value}` (105.6px) in the surface accent.
- **Default weight for any headline**: 900.
- **Default weight for any body element**: 400 (lead = 500).

When unsure between sizes, lean larger. Studio's identity depends on type running massive — a 92.2px h2 reads as the system; a 53.8px h3 used as a primary headline reads as a different system.

### Signature Treatments

These treatments are **non-optional whenever the corresponding element type is used**:

- **Every display, h1, h2, h3, quote-text, and stat-value is uppercase.** Sentence-case display type does not exist. Even h3 (53.8px) at weight 700 runs uppercase.
...
Studio targets `1920px × 1080px` — full viewport. Each `.slide` flexes to fill the viewport exactly, and slides sit side-by-side in a horizontal strip that translates left/right on navigation. All sizes use viewport-relative units (`vw`, `vh`) so layout scales fluidly.
...
| `{spacing.gap-lg}` | 37.8px | Between major content sections |
| `{spacing.gap-md}` | 21.6px | Between related elements |
| `{spacing.gap-sm}` | 10.8px | Between tightly coupled elements |
...
- A single display headline (typically one word at 230.4px) sits in the top area as `cover-type`.
...
**Display headline as graphic mass** — Barlow at weight 900 at display scale (92.2px and above) sits as a black or yellow geometric block. The headline is the primary decorative element of every slide; there are no additional ornaments.
...
**Stat card** — a flat region with a 2px top rule, a stat numeral (105.6px Barlow 900) in the surface accent, a stat label (22.1px Barlow 500), and an optional mono stat-note (16.3px IBM Plex Mono at tier-3 opacity). Padding right and bottom; left padding is zero (the rule starts flush with the card's left edge).
...
- Display headline scales: 230.4px → ~230px at 1920px viewport, ~154px at 1280px.
- Body text scales: 22.1px → ~22px at 1920px, ~15px at 1280px.
- Padding scales: pad-x 96px → ~96px at 1920px, ~64px at 1280px.
...
|---|---|---|---|---|
| Display / h1 / h2 / quote-text / stat-value | Barlow 900 uppercase | 思源宋体 / Noto Serif SC | 700 | Studio's defining "type-as-graphic-mass" effect depends on a single typeface at maximum weight. NSC 700 is the heaviest Song-style serif on CDN that holds visual mass at 230.4px / 144px / 92.2px display sizes. |
...
- **Line-height**: body 1.75–1.85 (Studio's 1.5–1.6 Barlow-default is fine for Latin but tight for CJK strokes); display 1.15–1.25 (looser than Latin 0.9–0.95 because CJK glyphs at 230.4px need vertical breathing).
...
Studio's binary palette (acid yellow on near-black, near-black on acid yellow) is perfectly preserved in CJK — no color decisions change. NSC 700 in `{colors.acid-yellow}` against the `{colors.near-black}` field at 230.4px display size carries the same visual punch as Barlow 900 in the same role; the saturated yellow against the warm dark surface makes Chinese characters read with the same poster-like authority.
...
Stat cards work well in CJK — the 105.6px italic-style stat numeral can render in NSC 700 if the value is a Chinese numeral (`三百万`) or stay in Barlow if Arabic (`3M`). The 2px top rule and the stat-note (mono) are unaffected.
...
- NSC 900 weight (which would better match Barlow 900) exists on Google Fonts but is heavy enough at large sizes that counter shapes start to close — at 230.4px display, NSC 900 may render some characters as near-solid blocks. NSC 700 is the recommended ceiling; for extreme display moments, test NSC 900 case-by-case.
...
6. Stat cards use the 2px top rule plus a 105.6px weight 900 numeral. Smaller stats, sentence-case stats, or rounded stat tiles break the pattern.
