...
spacing:
  frame-inset: "69.1px"
  frame-inset-bottom: "75.6px"
  topbar-gap: "15.1px"
  card-pad-x: "20px"
  card-pad-y: "24px"
  grid-gap: "26px"
  col-gap: "56px"
  pagenum-inset: "23.8px 42.2px"
...
This model was authored fluid for a ~1088 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.765** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.

| role | model (frozen) | **on canvas — use this** |
|---|---|---|
| `disp-hero` | 237.6 px | **419 px** |
| `num-hero` | 194.4 px | **343 px** |
| `disp-statement` | 151.2 px | **267 px** |
| `disp-title` | 151.2 px | **267 px** |
| `disp-lockup` | 118.8 px | **210 px** |
| `num-md` | 118.8 px | **210 px** |
| `disp-quote-lg` | 113.4 px | **200 px** |
| `disp-section` | 97.2 px | **172 px** |
| `disp-quote` | 97.2 px | **172 px** |
| `disp-brand` | 56 px | **99 px** |
| `disp-card-name` | 48 px | **85 px** |
| `seal-text` | 38 px | **67 px** |
| `ttl-row` | 30 px | **53 px** |
| `stamp-text` | 28 px | **49 px** |
| `body-emphasis` | 20 px | **35 px** |
| `micro-spec` | 20 px | **35 px** |
| `body` | 17 px | **30 px** |
| `mono-md` | 16 px | **28 px** |
| `body-md` | 15 px | **26 px** |
| `micro` | 14 px | **25 px** |
| `micro-lg` | 14 px | **25 px** |
| `micro-xl` | 14 px | **25 px** |
| `mono-tag` | 14 px | **25 px** |
| `mono` | 12 px | **21 px** |
...
⚠️ Even re-anchored, these roles land under 24 px: mono. Chrome only — never reading text.
...
### Canvas System
The system targets `1920px × 1080px`. The deck is wrapped in a `.deck` grid with a centered `.stage` filling the viewport. Each `.slide` is absolutely positioned inset 0 with opacity 0; only the `.active` slide has opacity 1. Transitions are 280ms ease opacity fades. Navigation is JS-driven via arrow keys, space, PageUp/Down, Home/End, and touch swipe.

### Frame Inset Pattern
Most content slides follow a frame-inset pattern: a `.frame` div sits inset from the slide edges by `69.1px` on top/left/right and `75.6px` at bottom (to clear the page number). Inside the frame, a topbar pattern (title + meta-label + 1.5px ink rule) sits above a body region.
...
The system targets `1920px × 1080px` and uses `clamp()` throughout with `min(Xvw, Yvh)` patterns that combine viewport-width and viewport-height constraints. This produces fluid scaling across desktop sizes without responsive breakpoints. There is no defined mobile breakpoint — the deck is presentation-first.
