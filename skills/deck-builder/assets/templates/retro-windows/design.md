...
|---|---|---|
| `text-xl` | 32 px | **60 px** |
| `metric-xl` | 30 px | **56 px** |
| `text-lg` | 22 px | **41 px** |
| `terminal` | 22 px | **41 px** |
| `text-md` | 18 px | **34 px** |
| `body` | 16 px | **30 px** |
| `nav-hint` | 16 px | **30 px** |
| `text-sm` | 14 px | **26 px** |
| `title-bar` | 14 px | **26 px** |
| `group-box-title` | 13 px | **24 px** |
| `text-xs` | 12 px | **22 px** |
...
⚠️ Even re-anchored, these roles land under 24 px: text-xs. Chrome only — never reading text.
...
### Canvas System
The system targets `1920px × 1080px` with each slide positioned `position: fixed; top: 0; left: 0`. Only the `.active` slide is `display: flex`; others are `display: none`. Inside each slide, a single `{components.win-window}` (or a few stacked windows) provides the framing chrome. The window is `max-width: 1200px` and `max-height: 1012px` so the desktop gray border is always visible around the chrome.
...
The system is **viewport-fluid** but **fixed-pixel**: the window chrome and body sizes flex with the viewport (`max-width: 1200px`, `max-height: 1012px`), but font-sizes are integer pixels, not clamped. A single media query at `max-width: 900px` reduces slide padding to 15px-20px and collapses 4-column grids to 2-column.

### Scaling Behavior
- The window grows to fill the viewport up to 1200×(1080px-68px), then stops scaling.
