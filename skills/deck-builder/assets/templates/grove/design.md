...
## Type scale on the 1920 canvas — USE THIS LADDER

This model was authored fluid for a ~1293 px stage; its reading sizes land under the house floor (28 px body). One factor **k = 1.485** re-anchors the ladder on the 1920×1080 canvas, anchored on `body` → 30 px. Ratios are preserved, so the model's hierarchy is intact.

⚠️ **Partial scale.** This model pairs small reading type with graphic-scale display type. Roles that would pass 420 px are left as written (marked *unchanged*) — they are already canvas-scale graphics, not text. Only the reading ladder moves.

| role | model (frozen) | **on canvas — use this** |
|---|---|---|
| `grove-num` | 345.6 px | **346 px** — unchanged |
| `display` | 192 px | **285 px** |
| `quote-mark` | 153.6 px | **228 px** |
| `h1` | 105.6 px | **157 px** |
| `stat-value` | 86.4 px | **128 px** |
| `h1-statement` | 81 px | **120 px** |
| `h2` | 61.4 px | **91 px** |
| `quote-text` | 61.4 px | **91 px** |
| `h3` | 38.4 px | **57 px** |
| `lead` | 27.8 px | **41 px** |
| `body-list-emph` | 26.9 px | **40 px** |
| `body` | 20.2 px | **30 px** |
| `caption` | 15.7 px | **23 px** |
| `label` | 13.4 px | **20 px** |
| `kicker` | 13.4 px | **20 px** |
| `chapter-num` | 13.4 px | **20 px** |
| `stat-label` | 13.4 px | **20 px** |

**Multiply the model's spacing, padding, radii, borders and component sizes by the same k = 1.485.** The exceptions are canvas-sized values: `1920px` (was `100vw`), `1080px` (was `100vh`) and anything derived from them stay as written — they are already expressed in canvas units.

Adjust the anchor, not the ratios: a dense reading deck may drop the body to 28 px, a speaker-led deck may lift it to 36 px. Never go under 28 px body / 26 px kicker / 24 px absolute (doctrine rule 4).

⚠️ Even re-anchored, these roles land under 24 px: caption, label, kicker, chapter-num, stat-label. Chrome only — never reading text.

## Fixed-stage policy — already applied
...
`clamp()`, `min()`, `max()` and `calc()` were resolved once (19.2px = 19.2px,
10.8px = 10.8px, 16px = 16px). **Use the numbers as written.** Do not re-derive
...
- A massive serif watermark numeral (`{components.grove-num}` at 345.6px, 6% opacity) sits in the bottom-right corner of chapter and section slides as compositional texture.
...
|---|---|---|---|---|
| `{typography.grove-num}` | 345.6px | Playfair | 400 | Watermark ordinal numeral, 6% opacity, absolute bottom-right |
| `{typography.display}` | 192px | Playfair | 400 | Cover hero title |
| `{typography.quote-mark}` | 153.6px | Playfair | 400 | Opening quotation glyph |
| `{typography.h1}` | 105.6px | Playfair | 400 | Chapter title, statement headline |
| `{typography.stat-value}` | 86.4px | Playfair | 400 / coral | Stat-card numeric value |
| `{typography.h2}` | 61.4px | Playfair | 400 | Routine slide headline |
| `{typography.quote-text}` | 61.4px | Playfair | 400 italic | Quote body |
| `{typography.h3}` | 38.4px | Playfair | 400 | Sub-headline, compare-panel title |
| `{typography.lead}` | 27.8px | Jost | 300 | Lead paragraph |
| `{typography.body-list-emph}` | 26.9px | Jost | 300 | List-slide body / bullets — emphasized for legibility |
| `{typography.body}` | 20.2px | Jost | 300 | Standard body paragraph |
| `{typography.caption}` | 15.7px | Jost | 300 | Captions, footnotes, chart sources |
| `{typography.label}` | 13.4px | JetBrains Mono | 300 / 0.12em | Chrome label, mono metadata |
| `{typography.kicker}` | 13.4px | JetBrains Mono | 300 / 0.14em / UPPER | Eyebrow above a headline |
| `{typography.chapter-num}` | 13.4px | JetBrains Mono | 300 / 0.2em / UPPER | Chapter ordinal label |
| `{typography.stat-label}` | 13.4px | JetBrains Mono | 300 / 0.12em / UPPER | Mono label beneath a stat figure |

### Defaults
- **Default size for a routine slide headline**: `{typography.h2}` (61.4px).
- **Default size for a chapter or statement headline**: `{typography.h1}` (105.6px). For statement-class moments specifically, use `{typography.h1-statement}` which caps at `81px` to prevent overflow on short viewports.
- **Default size for a cover hero title**: `{typography.display}` (192px).
- **Default size for a body paragraph**: `{typography.body}` (20.2px) in Jost weight 300.
- **Default size for a lede / introductory paragraph**: `{typography.lead}` (27.8px).
- **Default size for a kicker / eyebrow**: `{typography.kicker}` (13.4px) in JetBrains Mono uppercase 0.14em with `{colors.accent}` color.
- **Default size for a stat-card figure**: `{typography.stat-value}` (86.4px) in Playfair coral.
...
When the body text on a list-style slide reads too small at 20.2px on typical laptop viewports, the system bumps it to `{typography.body-list-emph}` (max of 26.9px or 17px). Reach for the emphasized size whenever a slide's primary content moment is a body paragraph or bulleted list — the default 20.2px is calibrated for projector viewing, not laptop reading.
...
- **The opening quote mark on a quote slide is rendered as a massive Playfair glyph at 153.6px in `{colors.accent}` coral.** A quote without the oversized coral quote-mark is broken.
...
### Canvas System
The system targets a fluid viewport — each `.slide` is `1920px × 1080px` with `padding: {spacing.pad-y} {spacing.pad-x}` (70.2px / 153.6px). Slides sit side-by-side in a horizontal `#deck` flex strip and translate via `transform: translateX(...)` with the `{motion.dur-slide}` / `{motion.ease-slide}` transition. Only one slide is `is-active` at a time; non-active slides hold their content invisible (`[data-anim]` is `opacity: 0`) until the active class triggers the entrance animations.

### Padding Scale
| Token | Value | Use |
|---|---|---|
| `{spacing.pad-x}` | 153.6px | Horizontal slide padding |
| `{spacing.pad-y}` | 70.2px | Vertical slide padding |
| `{spacing.pad-quote-x}` | 169px | Slightly wider horizontal padding on quote slides |
| `{spacing.pad-quote-y}` | 84.2px | Slightly taller vertical padding on quote slides |
| `{spacing.gap-lg}` | 48.6px | Between major content sections |
| `{spacing.gap-md}` | 30.2px | Between related elements |
| `{spacing.gap-sm}` | 15.1px | Between tightly coupled elements |
...
**Grove Stat** (`{components.grove-stat}`) — A vertical stack: large Playfair coral value (86.4px) on top, mono uppercase label beneath, 1px border-bottom hairline. No background fill — the card is defined by the rule + the type, not by a containing box.

**Quote Mark** (`{components.quote-mark}`) — Massive Playfair opening-quote glyph at 153.6px in coral, placed above the italic quote body. Always present on a quote slide.

**Watermark Numeral** (`{components.grove-num}`) — Playfair digit at 345.6px / 6% opacity in the bottom-right corner. Compositional texture only, never UI.
...
### Scaling Behavior
- Display headline scales linearly with viewport width (192px → 192px at 1920, 128px at 1280).
- Body text scales similarly (20.2px → 20.16px at 1920, 13.44px at 1280) — except on list-slide body, where the `26.9px` floor prevents the text from going unreadable on smaller viewports.
...
- The horizontal `#deck` strip translates via `transform: translateX(-N * 1920px)` between slides, with the `{motion.dur-slide}` sharp deceleration curve at 0.9s.
...
|---|---|---|---|
| Display / h1 / h2 / h3 (Playfair roles, 2–192px) | 霞鹜文楷 LXGW WenKai | 400 | Literary, hand-set warmth that mirrors Playfair Display at weight 400 — never bold, exactly the Grove rule |
| Quote text / mark (153.6px mark, 61.4px text) | 霞鹜文楷 LXGW WenKai | 400 | The same literary warmth carries the italic-quote moment; Chinese has no italic so the face does the expressive work |
| Stat figure (86.4px, coral) | 霞鹜文楷 LXGW WenKai | 400 | Keeps stat figures in the Playfair register; the coral color carries the accent |
| Watermark numeral (345.6px) | 霞鹜文楷 LXGW WenKai | 400 | The 6%-opacity watermark works at this scale with LXGW WenKai's open letterforms |
| Body / lede (Jost roles, 1.05–27.8px) | 思源宋体 Noto Serif SC | 300–400 | Mincho body voice — calm, literary, sits back like Jost weight 300 |
...
Baseline mismatch at display sizes (5.5–192px) is mild — LXGW WenKai and Playfair Display both sit at similar optical baselines, so mixed-script headlines like `A Quiet 山林` read cleanly. The `<em>` italic-coral accent rule is the more delicate piece (see Known CJK Gap below).
...
The watermark numeral (345.6px, 6% opacity) is particularly effective in Chinese — render a Chinese ordinal character (e.g., 「三」or 「五」) instead of a Western digit. At 6% opacity it reads as faint compositional texture, and the Han glyph's denser visual weight at that scale balances the slide better than a thin Western numeral would.
...
1. Any new headline is Playfair Display weight 400. Pick the size from the headline ladder (192px display / 105.6px h1 / 61.4px h2 / 38.4px h3) — do not invent a new size.
...
3. Any new body paragraph is Jost weight 300 at 20.2px (or 27.8px for lede, or `26.9px` on list-style slides).
...
- The grove-num watermark uses 345.6px font-size — on extremely wide viewports (3000px+) this becomes very large and may push the slide-foot area. The CSS positions it at `bottom: -0.15em` to absorb some overflow, but very tall numerals at very wide viewports may need adjustment.
