...
description: >-
  ...
  collaborateur (souvent depuis un fork) avant merge dans main. Utilise-le quand Leo dit « un
  ...
  veut un workflow de revue rapide — et AUSSI quand il demande à VOIR la contribution :
  ...
  (npm run audit : lint + secrets + tests + build + bundle), audit de code par sous-agent, banc
  de démonstration isolé + page HTML de revue à captures réelles, commune à Leo et au contributeur,
...
(déploiement automatique — vérifier lequel : `git remote -v`, ce n'est pas Netlify partout,
bwealthy passe par le remote `vpsprod`). Toute revue se fait sur une **branche `review/*` dans
un worktree séparé** (un 2ᵉ dossier ; le checkout principal reste sur `main`, intact). Le merge
dans `main` n'arrive **qu'une fois tout vert et validé par Leo**. Pas de « pousser sur main puis
...
compris contre le travail non mergé de Leo (§2, §2bis)** → worktree de revue → **`npm run
audit`** → **audit de code (sous-agent)** → **banc + HTML de revue (§6)** → **test manuel**
...
Deux erreurs de cadrage tuent la revue avant qu'elle commence : **auditer du code qui n'est pas
de lui** (§1bis), et **ne comparer qu'à `main`** en ignorant ce que Leo a en attente (§2bis) —
ce qui fait valider un travail qui sera à refaire dans deux semaines.
...
On audite **le travail du contributeur**, pas celui de Leo. Une branche de fork contient souvent
...
git log --format='%h %an %s' $MB..<collab>/<branche> \
  --perl-regexp --author='^(?!Antoine Trubert|gobien53)'
```
...
Vérifie aussi que les branches « en attente » côté `origin` sont bien de Leo : dans bwealthy,
`origin/review/traindevie-antoine` porte le nom d'Antoine et contient SES commits — la ranger
avec le travail de Leo fausserait le croisement de §2bis.
...
Un compteur d'occurrences (`git grep -c "train de vie" main` → 37) ne prouve **rien** : il dit que le
...
for k in "Solde à la retraite" "cfMensuel" "methodTab"; do
...
### 2bis. Croiser avec le travail non mergé de Leo — les collisions à venir
`merge-tree` contre `main` ne voit qu'une partie du risque. Ce qui fera vraiment mal, c'est la
branche de Leo **encore en attente** qui touche les mêmes zones : au moment où elle atterrira,
le travail du contributeur sera à refaire. Il faut le dire **avant** de lui demander un rebase.
```bash
# Ce que Leo a en attente : branches non mergées + WIP non commité
...
```
Mesuré sur bwealthy : le merge contre `main` seul annonçait 7 fichiers en conflit, mais
`fix/mcp-schemaversion-dataloss` et `wip/powens-mac` en ajoutent un 8ᵉ (`src/lib/regimesImmo.js`)
— invisible tant qu'on ne croise pas. Ce genre de fichier doit partir dans le message au
contributeur comme « zone à ne pas retoucher / à rebaser en dernier ».

Restituer en tableau, une ligne par branche en attente :

| Branche de Leo en attente | Commits | Fichiers en commun | Conflit réel | Ce qu'on demande au contributeur |
|---|---|---|---|---|
| `wip/powens-mac` | 8 | 6 | ⚠️ `regimesImmo.js` en plus | ne pas toucher ce module, ou rebaser après |
...
Quand Leo demande à voir (« fais-moi un HTML », « montre-moi ce que ça donne »), la réponse
...
d'un coup tous les conflits (17 blocs sur la contribution d'Antoine) :
```bash
...
peut bumper `SCHEMA_VERSION` et l'autosave réécrira des dossiers clients réels dans le
nouveau format. Deux gestes :
```bash
cat > .env <<'EOF'
VITE_SUPABASE_URL=http://127.0.0.1:9        # port fermé → échec immédiat, pas de timeout
VITE_SUPABASE_ANON_KEY=demo-non-fonctionnelle
VITE_DEMO_LOCAL=1
EOF
```
puis, dans le worktree seulement, un court-circuit gardé par `VITE_DEMO_LOCAL` : `AuthProvider`
...
- **Les modules filtrent leurs items.** Un bien n'apparaît pas parce qu'un drapeau manque
  (`analyseRentabiliteActive`). Lire le collecteur (`isActive`, `aggregate*`) et les champs
...
- **Certains onglets sont gatés `isAdmin`.** Passer le profil démo en `admin` pour les voir…
  mais l'app atterrit alors sur l'écran d'administration, vide sans Supabase : forcer l'onglet
  initial sur la synthèse en mode démo.
...
**UNE seule page, adressée au contributeur, que Leo lit par-dessus son épaule.** C'est la forme
arbitrée par Leo (04/08/2026) après avoir essayé la version « une page pour lui, une pour l'autre » :
deux pages qui racontent la même chose divergent au premier correctif, et Leo n'a pas de raison de
...
12. **Un encart de fin** : les zones à ne pas toucher (§2bis), le rappel que plusieurs points sont
    discutables, et l'état d'envoi (rien n'est parti tant que Leo n'a pas dit oui).
...
## 7. Test manuel par Leo (le « avant » du merge)
```bash
...
`revue-<collab>.md` **hors du dépôt** (`~/Documents/Claude/Projects/…`) :
```bash
D=~/Documents/Claude/Projects/<repo>-revue-<collab>   # un DOSSIER : le registre y côtoiera
...
|---|---|---|
| la page HTML de §6 (à captures, voyants) | **Leo ET le contributeur** | comprendre et décider |
| `revue-<collab>-POUR-<COLLAB>.md` | le contributeur **et son assistant de code** | **corriger** |
| le registre de §9.2 | Leo, entre les tours | **suivre** les statuts |
...
- **Ne jamais lui faire inventer une valeur.** Si on n'a pas la source, on demande un marqueur,
  pas une correction. Une correction plausible mais fausse est pire que le doute documenté.
...
Ne jamais envoyer sans l'accord de Leo : c'est un message sortant, vers un tiers.

### 9.4 Le critère de sortie, annoncé dès le premier tour
Sans critère écrit, la boucle ne se ferme jamais. Feu vert = les cinq à la fois :
- zéro `ouvert` en sévérité BLOQUANT ;
- les IMPORTANT sont soit `corrigé`, soit `reporté` avec une raison acceptée par Leo ;
...
- **Le banc de démonstration ne touche jamais une base réelle** : env injoignable + autosave coupé.
  ...
  des dossiers clients pour une capture d'écran.
- Les patchs du banc (auth court-circuitée, `demoState.js`, onglet forcé) **ne sont jamais commités**.

## Orchestration
- Charge `verify` en clôture : la preuve qu'on rend est le livrable que Leo review, il ne
  retestera pas lui-même. Ici la preuve, ce sont les captures — donc jamais de maquette.
- Si la restitution HTML mérite un vrai travail de mise en page, `bestfront` porte la boucle
  de vérification visuelle et `visual-lab` la matière déjà validée.
